...
wordlist = np.load("res/rank_lista.npy")
...
def dopasowywanie_frazy(user_input, wordlist=wordlist, text=source_text, N=1,
                        l=0):  # (to wpisuje user, wordlista, porcja tekstu, ile wynikow syswietlic)
    wyniki = []
    if user_input:
        wordlist = [x for x in wordlist if
                    x.startswith(user_input.lower())]  # tutaj jeśli ma zaczynać sie na user_input
    ...
    for word in wordlist:
        ...
        if wynik:
            wyniki.append([word, int((wynik[-1] - l) / len(wynik)),
                           wynik])  # [słowo; stosunek zajmowanego miejsca do dlugosci frazy (the less the better); array indeksow]
